...
flexarray_t new_flexarray(size_t member_size, size_t initial_alloc, allocator_t* alloc, deallocator_t* dealloc) {
    ...
    flexarray_t res  = alloc(sizeof(struct flexarray));
    res->member_size = member_size;
    res->alloc       = initial_alloc;
    res->count       = 0;
    res->allocator   = alloc;
    res->deallocator = dealloc;
    res->data        = alloc(initial_alloc * member_size);

    return res;
}

void delete_flexarray(flexarray_t array) {
    array->deallocator(array);
}
...
static void flexarray_grow(flexarray_t array, size_t copy_offset) {
    ...
    void* new_data = array->allocator(new_size);
    memcpy(new_data, array->data + (copy_offset * array->member_size), old_size);
    array->deallocator(array->data);
