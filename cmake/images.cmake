...
math(EXPR bootfs_start_sectors "2048")
math(EXPR bootfs_start "${bootfs_start_sectors} * 512")
math(EXPR bootfs_sectors "${image_size_hd_sectors} - ${bootfs_start_sectors}")
math(EXPR bootfs_physical_drive "${bootfs_start} + 64")

add_custom_target(hd.img
    COMMAND ${dd} if=/dev/zero of=hd.img bs=512 count=${image_size_hd_sectors}
    COMMAND ${sgdisk}  -Z -o -n 1:${bootfs_start_sectors}:${image_size_headers} -t 1:ef00 hd.img -A 1:set:2
    COMMAND ${mformat} -i hd.img@@1M -F -L ${bootfs_sectors} -H ${bootfs_start_sectors} -B ${CMAKE_SOURCE_DIR}/packaging/CloverV2/boot1f32
    COMMAND ${mcopy}   -i hd.img@@1M -sbnmv ${CMAKE_BINARY_DIR}/shared/bootfs/* ::/

    COMMAND ${mcopy} -i hd.img@@1M -sbnmv ${CMAKE_SOURCE_DIR}/packaging/CloverV2/boot6 ::/boot
    COMMAND ${dd} if=${CMAKE_SOURCE_DIR}/packaging/CloverV2/boot0af of=hd.img bs=1 count=440 conv=notrunc
)
